...
export const baseGeneralSettingsSchema = z.object({
  ...
  headerLogoUrl: z.string()
    ...
    .optional()
    .nullable()
    .refine((val) => !val || val.startsWith('/objects/') || val.startsWith('/uploads/') || val.startsWith('/api/logo/') || val.startsWith('http'), {
      message: "Logo-URL muss eine gültige HTTP-URL, Object-Storage-Pfad, Datenbank-Logo-Pfad oder lokaler Upload-Pfad sein",
    }),
