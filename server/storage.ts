...
import { generalSettingsSchema } from "@shared/schema";
import type {
  UrlRule,
  InsertUrlRule,
  UrlTracking,
...
const GENERAL_SETTING_CATEGORIES: Record<string, string> = {
  id: "metadata",
  updatedAt: "metadata",
  headerTitle: "header",
  headerIcon: "header",
...
function getGeneralSettingCategory(key: string): string {
  return GENERAL_SETTING_CATEGORIES[key] ?? "custom";
}

function normalizeGeneralSettings(settings: Partial<GeneralSettings>, id: string): GeneralSettings {
  ...
  const normalized = {
    ...
    ...knownSettings,
    id,
    smartSearchRules: knownSettings.smartSearchRules ?? (
      knownSettings.smartSearchRegex ? [{ pattern: knownSettings.smartSearchRegex, order: 0 }] : defaults.smartSearchRules
    ),
    updatedAt: knownSettings.updatedAt ?? defaults.updatedAt,
  };

  return generalSettingsSchema.parse(normalized);
}
