...
## 2025-05-01 - CSV Injection Vulnerability in Statistics Export
**Vulnerability:** The statistics export functionality (`/api/admin/export` route) in `server/routes.ts` did not sanitize user-provided tracking fields (like `oldUrl`, `path`, `referrer`, `userAgent`, `feedback`, `userProposedUrl`) before formatting them into a CSV string. This exposed administrators to "Formula Injection" if they opened the CSV in spreadsheet software like Excel, where malicious strings starting with `=`, `+`, `-`, or `@` could execute arbitrary formulas.
**Learning:** Even fields meant for analytical tracking (such as URLs or user agents) are user-controlled input and must be sanitized when rendered in sensitive formats like CSV/Excel. The previously implemented `sanitizeForCSV` function only protected rule exports but neglected tracking data.
**Prevention:** Sanitized all potentially user-controlled fields with `ImportExportService.sanitizeForCSV()` within the `trackingData` mapping logic before generating the CSV row strings. Ensure that *every* CSV export mechanism across the application utilizes this sanitizer.
